...
(function () {
    ...
    function Runner(outerContainerId, opt_config) {
        ...
        // Merge over the defaults so embedding config (callbacks, etc.)
        // never wipes out the game tuning (SPEED, GRAVITY, CLEAR_TIME...).
    ...
    Runner.prototype = {
        ...
        init: function () {
            ...
            this.onResizeBound_ = this.debounceResize.bind(this);
        ...
        startGame: function () {
            ...
            this.onVisibilityBound_ = this.onVisibilityChange.bind(this);
            document.addEventListener(Runner.events.VISIBILITY,
                this.onVisibilityBound_);
        ...
        startListening: function () {
            ...
            if (IS_MOBILE) {
                ...
                // Mouse (scoped to the game card, not the whole page).
        ...
        gameOver: function () {
            ...
            if (this.config.onGameOver) {
                // Match the meters shown on the in-game distance meter.
                var finalScore = this.distanceMeter.getActualDistance(
                    Math.ceil(this.distanceRan));
                this.config.onGameOver(finalScore);
            }
         ...
         */
        setArcadeMode() {
            // Embedded mode: scale to the card, never touch the page body.
            this.setArcadeModeContainerScale();
        },
        ...
        setArcadeModeContainerScale() {
            const containerWidth = this.outerContainerEl ?
                this.outerContainerEl.offsetWidth : DEFAULT_WIDTH;
            const scale = Math.min(1, containerWidth / this.dimensions.WIDTH);
            const scaledWidth = Math.round(this.dimensions.WIDTH * scale);
            if (this.containerEl) {
                this.containerEl.style.width = scaledWidth + 'px';
                this.containerEl.style.transform = 'scale(' + scale + ')';
                this.containerEl.style.transformOrigin = 'top center';
            }
        },
        ...
        /**
         * Tear the game down so it can be safely removed from the DOM.
         */
        destroy: function () {
            this.stopListening();
            if (this.onResizeBound_) {
                window.removeEventListener(Runner.events.RESIZE, this.onResizeBound_);
            }
            if (this.onVisibilityBound_) {
                document.removeEventListener(Runner.events.VISIBILITY,
                    this.onVisibilityBound_);
                window.removeEventListener(Runner.events.BLUR, this.onVisibilityBound_);
                window.removeEventListener(Runner.events.FOCUS, this.onVisibilityBound_);
            }
            clearInterval(this.resizeTimerId_);
